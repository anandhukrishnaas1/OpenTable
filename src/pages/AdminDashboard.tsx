...
const AdminDashboard: React.FC = () => {
  ...
  const getStatusBadge = (status: ApplicationStatus) => {
    switch (status) {
      case 'Pending':
        return (
          <span className="px-2 py-1 bg-yellow-100 text-yellow-700 rounded-md text-xs font-bold ring-1 ring-yellow-200">
            Pending
          </span>
        );
      case 'Verified':
      ...
      case 'Flagged':
        return (
          <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded-md text-xs font-bold ring-1 ring-orange-200">
            Flagged
          </span>
        );
      case 'Rejected':
  ...
  const pendingCount = applications.filter(
    (a) => a.status === 'Pending' || a.status === 'Flagged'
  ).length;
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 border-r border-gray-200 pr-4">
            ...
            <div className="flex bg-gray-100/80 p-1.5 rounded-full mb-6 shadow-sm border border-gray-100">
              ...
              >
                Approved{' '}
                <span className="ml-1 bg-green-500 text-white text-[10px] px-2 py-0.5 rounded-full">
                  {approvedCount}
                </span>
              </button>
              <button
            ...
            {selectedApp ? (
              <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50 animate-fade-in">
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-6 border-b border-gray-100 pb-8 mb-8">
                  <div>
                    ...
                    <p className="text-gray-500 font-medium">
                      {selectedApp.email}{' '}
                      <span className="mx-2 flex-inline items-center justify-center text-gray-300">
                        •
                      </span>{' '}
                      <span className="text-xs bg-gray-100 px-2 py-1 rounded-md text-gray-500 tracking-widest font-mono">
                        ID: {selectedApp.id}
                      </span>
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    {selectedApp.status === 'Verified' ? (
                      <button
                        ...
                        className="flex items-center gap-2 bg-red-50 hover:bg-red-100 text-red-700 px-5 py-2.5 rounded-full font-bold transition-colors shadow-sm"
                      >
                      ...
                      <>
                        <button
                          onClick={() => updateApplicationStatus(selectedApp.id, 'Verified', 50)}
                          className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 rounded-full font-bold transition-colors shadow-md shadow-green-600/20"
                        >
                          <Check size={18} /> Verify & Approve
                        </button>
                        <button
                          onClick={() => updateApplicationStatus(selectedApp.id, 'Flagged')}
                          className="flex items-center gap-2 bg-orange-50 hover:bg-orange-100 text-orange-700 px-5 py-2.5 rounded-full font-bold transition-colors shadow-sm"
                        >
                ...
                <div className="grid md:grid-cols-2 gap-8 mb-8">
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl text-sm border border-gray-200 min-h-[100px]">
                      {selectedApp.backgroundCheckDeclaration || (
                        <span className="text-gray-400 italic">No declaration provided.</span>
                      )}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl text-sm border border-gray-200 font-mono">
                      {selectedApp.organizationAffiliation || (
                ...
                <div className="flex flex-wrap gap-4 mb-8">
                  <div
                    className={`flex items-center gap-2 px-4 py-3 rounded-xl border ${selectedApp.termsSigned ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}
                  >
                    {selectedApp.termsSigned ? (
                  ...
                  {selectedApp.status === 'Verified' && (
                    <div className="px-4 py-3 bg-blue-50 border border-blue-200 text-blue-800 rounded-xl flex items-center gap-2 text-sm font-bold">
                      <Shield size={20} className="text-blue-600" /> Initial Trust Score:{' '}
                      {selectedApp.trustScore} / 100
                    </div>
                  )}
                </div>

                {/* Side-by-side Document Viewer */}
                <h3 className="font-bold text-gray-700 mb-4 uppercase text-xs tracking-wider border-t pt-6">
                  Identity Cross-Reference Viewer
                </h3>
                <div className="grid md:grid-cols-2 gap-6 bg-gray-100 p-6 rounded-2xl border border-gray-200">
                  <div>
                    <span className="block text-center text-sm font-bold text-gray-500 mb-3">
                      Govt Issued ID
                    </span>
              ...
              <div className="h-full min-h-[600px] flex flex-col items-center justify-center text-gray-400 bg-gray-50 rounded-[2.5rem] border border-gray-200 border-dashed">
                ...
                <p className="text-gray-500 mt-2 font-medium">
                  Rigorous onboarding builds trust across our platform.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Review proof photos and applaud your volunteers
              </p>
            </div>
          </div>
          ...
          {completedDeliveries.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {completedDeliveries.map((delivery) => (
                <div
                  key={delivery.id}
                  className="bg-white rounded-[2rem] p-6 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 hover:border-green-200 transition-all"
                >
                  ...
                  <p className="text-xs text-gray-400 mt-1 truncate">
                    Delivered by: {delivery.claimedBy}
                  </p>
