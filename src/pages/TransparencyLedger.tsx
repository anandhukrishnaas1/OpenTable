...
const parseKg = (qty: string): number => {
  ...
  if (match) {
    ...
    if (lower.includes('plate') || lower.includes('meal') || lower.includes('serving'))
      return num * 0.4;
    if (lower.includes('piece') || lower.includes('pcs') || lower.includes('item'))
      return num * 0.2;
...
const TransparencyLedger: React.FC = () => {
  ...
  donations
    .filter((d) => d.status === 'delivered' && d.claimedBy)
    .forEach((d) => {
      if (d.claimedBy) {
  ...
  const topVolunteers = Object.values(volStats)
    ...
    .map((vs) => {
      ...
      const name = app
        ? app.name.split(' ')[0] + ' ' + (app.name.split(' ')[1]?.[0] || '') + '.'
        : vs.email.split('@')[0];
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-16 relative">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto font-medium">
            We believe in total accountability. Every kilogram of food donated, every meal served,
            and every volunteer verifying a drop-off is tracked here for the community to see.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          ...
          <div className="bg-white rounded-[2.5rem] p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50 flex flex-col items-center text-center transition-transform hover:-translate-y-1">
            ...
            <div className="text-5xl font-black text-gray-900 mb-2 tracking-tight">
              {activeVolunteersCount.toLocaleString()}
            </div>
            <div className="text-gray-500 font-bold uppercase tracking-wider text-sm">
              Active Volunteers
            </div>
            ...
            <div className="space-y-4">
              {topDonors.length > 0 ? (
                topDonors.map((donor, idx) => (
                  <div
                    key={idx}
                    className="flex items-center p-5 bg-gray-50/80 rounded-[1.5rem] transition-colors hover:bg-gray-100"
                  >
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500 flex items-center gap-1.5 font-medium mt-0.5">
                        {vol.rating}{' '}
                        <Star size={14} fill="currentColor" className="text-yellow-500" /> Avg
                        Rating
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="block font-black text-gray-900 text-2xl tracking-tight leading-none">
                        {vol.deliveries}
                      </span>
        ...
        <div className="mt-16 text-center text-sm text-gray-500 bg-gray-50 p-6 rounded-2xl border border-gray-100">
          Note: To protect recipient privacy, real-time map tracking data is only visible to the
          specific donor and volunteer assigned to an active delivery ticket. This public ledger
