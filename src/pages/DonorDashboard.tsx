        ...
        <div className="bg-gray-100/80 rounded-full p-1.5 mb-10 flex shadow-sm border border-gray-100">
          {['scan', 'active', 'history'].map((tab) => (
            <button
              ...
              className={`flex-1 py-3 px-6 rounded-full text-sm font-bold transition-all capitalize ${
                activeTab === tab
          ...
          <div className="space-y-6">
            ...
            {!image && !isCameraOpen && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <button
                  onClick={startCamera}
                  className="bg-green-600 text-white rounded-[2rem] p-10 flex flex-col items-center justify-center gap-4 hover:bg-green-700 transition-all shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1"
                >
                ...
                <label className="bg-white border-2 border-dashed border-gray-200 rounded-[2rem] p-10 flex flex-col items-center justify-center gap-4 hover:border-green-400 hover:bg-green-50 transition-all cursor-pointer shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                  <input
                    ...
                    className="hidden"
                  />
                  <div className="bg-green-50 p-5 rounded-full text-green-600">
                    <Upload size={48} />
                  </div>
              ...
              <div className="relative bg-black rounded-2xl overflow-hidden shadow-xl">
                ...
                <div className="absolute bottom-6 left-0 right-0 flex justify-center items-center gap-8">
                  ...
                  <button
                    ...
                    className="bg-white p-1 rounded-full border-4 border-gray-200 hover:scale-105 transition-transform"
                  >
              ...
              <div className="bg-white rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden border border-gray-100">
                <div className="relative h-72 bg-gray-100">
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 bg-white/90 p-2 rounded-full text-gray-600 hover:text-red-600"
                  >
                    <RefreshCw size={20} />
                  </button>
                  {analyzing && (
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div className="border-t pt-6">
                      ...
                      <input
                        ...
                        className="w-full mt-4 p-3 bg-gray-50 border rounded-xl"
                    ...
                    <div className="border-t pt-8">
                      ...
                      <button
                        ...
                        className="w-full bg-green-50 hover:bg-green-100 text-green-700 font-bold py-4 rounded-full mb-4 transition-colors"
                      >
                      ...
                      <input
                        ...
                        className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-shadow"
                    ...
                    <div className="pt-4">
                      <button
                        ...
                        className="w-full bg-gray-900 hover:bg-black text-white font-bold py-4 rounded-full shadow-lg transition-transform hover:-translate-y-0.5 disabled:bg-gray-400 disabled:cursor-not-allowed"
                      >
                        {isSubmitting ? 'Saving...' : 'Confirm Donation'}
                      </button>
                    </div>
                  </div>
                )}
                {scanResult && scanResult.safeToEat === 'No' && (
                  <div className="p-6 bg-red-50 text-red-700 font-bold text-center">
                    Item Unsafe to Donate
                  </div>
                )}
              </div>
            )}
        ...
        {activeTab !== 'scan' && (
          <div className="space-y-4">
            {(activeTab === 'active' ? activeDonations : historyDonations).map((item) => (
              <div
                key={item.id}
                className="bg-white p-6 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 flex gap-5 transition-all hover:border-gray-200"
              >
                {item.imageUrl && (
                  <img
                    src={item.imageUrl}
                    className="w-24 h-24 rounded-2xl object-cover bg-gray-100 shrink-0"
                  />
                )}
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    <span
                      className={`text-xs font-bold px-3 py-1.5 rounded-full ${item.status === 'active' ? 'bg-orange-50 text-orange-700' : 'bg-green-50 text-green-700'}`}
                    >
                  ...
                  <p className="text-sm text-gray-500 mt-3 flex items-center gap-1.5 truncate">
                    <Package size={14} /> {item.quantity} • <MapPin size={14} /> {item.address}
                  </p>
                </div>
              </div>
            ))}
            {activeTab === 'active' && activeDonations.length === 0 && (
              <p className="text-center text-gray-400 py-10">No active donations.</p>
            )}
            {activeTab === 'history' && historyDonations.length === 0 && (
